...
def test_assignment_submit(file_path):
    time.sleep(2)
    ...
    if is_element_clickable("//div[3]/div/div/span/button"):
        ...
        webdriver.ActionChains(driver).send_keys(Keys.ESCAPE).perform()
        XLUtils.writeData(path, "UploadAssignmentDecision", r, 3, "FAILED")

    else:
        XLUtils.writeData(path, "UploadAssignmentDecision", r, 3, "PASSED")
...
rows = XLUtils.getRowCount(path, "UploadAssignmentDecision")

# Test assignment 1
for r in range(2, rows + 1):
    choice = XLUtils.readData(path, "UploadAssignmentDecision", r, 1)

    time.sleep(1)
    if choice == 1:
        rela_path = XLUtils.readData(path, "UploadAssignmentDecision", r, 2)
        file_path = get_file_path(rela_path)

        test_assignment_submit(file_path)
